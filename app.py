...
async def start():
    await cl.Message(
        content="Hi, Welcome to ShopMe! I’m here to assist you with order tracking and related queries. Tap 🎙️ to talk!"
    ).send()
...
@cl.on_message
async def on_message(message: cl.Message):
    # Retrieve the RealtimeClient instance
    openai_realtime: RealtimeClient = cl.user_session.get("openai_realtime")

    # Initialize and connect the RealtimeClient if not already connected
    if not openai_realtime:
        await setup_openai_realtime(system_prompt=system_prompt + "\n\nCustomer ID: 12121")
        openai_realtime = cl.user_session.get("openai_realtime")

    if openai_realtime and not openai_realtime.is_connected():
        await openai_realtime.connect()

    # Send the user message if connected
    if openai_realtime and openai_realtime.is_connected():
        await openai_realtime.send_user_message_content(
            [{"type": "input_text", "text": message.content}]
        )
    else:
        await cl.Message(content="Failed to connect to the Realtime client. Please try again.").send()
...
async def on_audio_start():
    try:
        ...
        # Ensure the RealtimeClient exists
        if not openai_realtime:
            await setup_openai_realtime(system_prompt=system_prompt + "\n\nCustomer ID: 12121")
            openai_realtime = cl.user_session.get("openai_realtime")
        ...
        if openai_realtime.is_connected():
            logger.info("RealtimeClient is already connected.")
            return True
