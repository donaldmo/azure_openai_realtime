...
class RealtimeAPI(RealtimeEventHandler):
    ...
    async def connect(self):
        ...
        self.ws = await websockets.connect(
            f"wss://{self.url.split('://')[-1]}/openai/realtime?api-version={
                self.api_version}&deployment={self.azure_deployment}&api-key={self.api_key}"
        )
...
class RealtimeConversation:
    ...
    def _process_speech_stopped(self, event, input_audio_buffer):
        ...
        if input_audio_buffer:
            start_index = (speech['audio_start_ms'] *
                           self.default_frequency) // 1000
            end_index = (speech['audio_end_ms'] *
                         self.default_frequency) // 1000
    ...
    def _process_output_item_added(self, event):
        ...
        if not response:
            raise Exception(f'response.output_item.added: Response "{
    ...
    def _process_output_item_done(self, event):
        ...
        if not found_item:
            raise Exception(f'response.output_item.done: Item "{
                            item["id"]}" not found')
    ...
    def _process_content_part_added(self, event):
        ...
        item = self.item_lookup.get(item_id)
        if not item:
            raise Exception(f'response.content_part.added: Item "{
                            item_id}" not found')
    ...
    def _process_audio_transcript_delta(self, event):
        ...
        item = self.item_lookup.get(item_id)
        if not item:
            raise Exception(f'response.audio_transcript.delta: Item "{
                            item_id}" not found')
    ...
    def _process_function_call_arguments_delta(self, event):
        ...
        item = self.item_lookup.get(item_id)
        if not item:
            raise Exception(f'response.function_call_arguments.delta: Item "{
                            item_id}" not found')
...
class RealtimeClient(RealtimeEventHandler):
    ...
    def _add_api_event_handlers(self):
        ...
        self.realtime.on("server.conversation.item.created",
        ...
        self.realtime.on(
            "server.response.audio_transcript.delta", self._process_event)
    ...
    def _process_event(self, event, *args):
        ...
        if event["type"] == "conversation.item.input_audio_transcription.completed":
            self.dispatch("conversation.item.input_audio_transcription.completed", {
                          "item": item, "delta": delta})
        if item:
            self.dispatch("conversation.updated", {
                          "item": item, "delta": delta})
        return item, delta
    ...
    async def add_tool(self, definition, handler):
        ...
        if name in self.tools:
            raise Exception(f'Tool "{name}" already added. Please use .removeTool("{
                            name}") before trying to add again.')
    ...
    def remove_tool(self, name):
        if name not in self.tools:
            raise Exception(
                f'Tool "{name}" does not exist, can not be removed.')
        del self.tools[name]
        return True
